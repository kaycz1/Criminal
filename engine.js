...
class InvestiEngine {
  constructor() {
    this.state = {
      ...
      suspicionLevels: {},
      suspectNotes: {},
      phoneUnlocked: false, // Se a senha 2023 do celular foi decifrada
  ...
  getMaxStageReached() {
    ...
    // Etapa 5 (Suspeitos) -> Uma vez acessada, abre Celular da Vítima
    if (this.state.currentStage >= 5) max = 6;

    // Etapa 6 (Celular da Vítima) → Interrogatório acessível após desbloqueio do telefone
    if (this.state.phoneUnlocked) max = 7;

    // Etapa 7 (Interrogatórios) → Acusação acessível após acesso
    if (this.state.currentStage >= 7) max = 8;
  ...
  // Validação da Senha do Celular (2023)
